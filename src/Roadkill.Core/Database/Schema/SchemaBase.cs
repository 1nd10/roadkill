...
namespace Roadkill.Core.Database.Schema
{
    public abstract class SchemaBase
    {
        ...
        protected string LoadFromResource(string resourcePath)
        {
            ...
            var stream = typeof(SchemaBase).Assembly.GetManifestResourceStream(resourcePath);
            if (stream == null)
            {
                throw new InvalidOperationException(string.Format("Unable to find '{0}' as an embedded resource",
                    resourcePath));
            }
